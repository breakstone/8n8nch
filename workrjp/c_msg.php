<?php
	...
	$all_nums = getObjectNum("dtb_people_msg","people_id='$url[user_id]'");
	if(strlen($url["user_id"]) <= 12){
		...
		if($url["user_type"] == "company"){
			...
			if($company){
				...
				$msgs = getMsg("people_id='$url[user_id]'",$start, $end);
				...
				$smarty->display('C/default/company_msg.html');
			}else{
				//没有找到
		...
		if($url["user_type"] == "user"){
			...
			if($user){
				$subPages = new SubPages($show_num,$all_nums,$pageCurrent,"?page=");
				//-----------------分页处理完了----------------------
				//利用limit查询数据库---select * from table limit $start,$end
				$start = $subPages->getStart($show_num,$pageCurrent);//配合分页得到起始数
				$end = $subPages->getEnd($all_nums,$show_num,$pageCurrent);//配合分页得到结束数
