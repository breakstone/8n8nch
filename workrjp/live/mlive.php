...
// 	echo $where;
	...
	//每页显示几条;//得到当前是第几页;
  	...
  	$subPages = new SubPages($show_num,$all_nums,$pageCurrent,10,APP_URL."live$sp/mlive/?searchB=true&keyword=$keyword&wanted=$want&type_url=$type_url&geju_url=$geju_url&juli=$juli&money_s=$money_s&money_e=$money_e&home_year=$home_year&mianji_s=$mianji_s&mianji_e=$mianji_e&weixiu_shangmen=$weixiu_shangmen&areaid=$areaid&pref=$pref&ensn_url=$ensn_url&eki_url=$eki_url&pid=");
	//-----------------分页处理完了----------------------
	...
	$smarty->display("mobile/Live/mlive.html");
	/*if($page!="index"){
		$smarty->display("mobile/Live/mlive.html");
	}else{
		$smarty->display("mobile/Live/live.html");
