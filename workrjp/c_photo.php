<?php
	...
	$sql = "select * from dtb_people_photo where del_flg = 0 and user_id='$url[user_id]' order by create_date";
	...
	if(strlen($url["user_id"]) <= 12){
		...
		if($url["user_type"] == "company"){
			...
			if($company){
				...
				$smarty->display('C/default/company_photo.html');
			}else{
				//没有找到
		...
		if($url["user_type"] == "user"){
			...
			if($user){
				...
				$smarty->assign('BASE_URL', APP_URL);
				$smarty->assign('THEME', THEME);
				$smarty->display('people_photo.html');
			}else{
				//没有找到
				$smarty->assign('h1', "信息提示");
