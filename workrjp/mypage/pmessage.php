<?php
	...
	$where = "message_to = '$_SESSION[kiwa_userid]'";
	$all_nums = getObjectNum("dtb_message",$where);
	//总条数

	//每页显示几条;//得到当前是第几页;
  	$show_num=10;$pageCurrent = $id;
  	$subPages = new SubPages($show_num,$all_nums,$pageCurrent,10,APP_URL."mypage/pmessage/");
	//-----------------分页处理完了----------------------
	...
	if(isMobile()){
		$smarty->display('mobile/Mypage/pmessage.html');
	}else{
		$smarty->display('Mypage/pmessage.html');
	}
