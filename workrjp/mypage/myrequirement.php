<?php
	...
	if(isset($_REQUEST["Which_Show"])&&$_REQUEST["Which_Show"]!=""){
		...
		if($_REQUEST["Which_Show"] == 2){
			...
			$table = "dtb_jobs";
			$where = "company_id = '$_SESSION[kiwa_userid]'";
		}
	}else{
	...
	//每页显示几条;//得到当前是第几页;
  	$show_num=15;$pageCurrent = $pages;
  	$subPages = new SubPages($show_num,$all_nums,$pageCurrent,10,APP_URL."mypage/myrequirement/$id/?Which_Show=$ws&pages=");
	//-----------------分页处理完了----------------------
	...
	//得到积分
	$sql_points="select points from dtb_user where user_id = '$_SESSION[kiwa_userid]' and del_flg = 0";
	...
	if(isMobile()){
		$smarty->display('mobile/Mypage/myrequirement.html');
	}else{
		$smarty->display('Mypage/myrequirement.html');
	}
