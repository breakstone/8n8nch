<?php
	...
	if($id == ""){$id = 1;}
	//总条数
	$all_nums = getObjectNum("dtb_job_lvli",$where);
	
	//每页显示几条;//得到当前是第几页;
  	$show_num=10;$pageCurrent = $id;
  	$subPages = new SubPages($show_num,$all_nums,$pageCurrent,10,APP_URL."mypage/sendnote/");
	//-----------------分页处理完了----------------------
	...
	$jianli = getJianli($where,$start,$end);
	...
	$smarty->display('Mypage/sendnote.html');
?>