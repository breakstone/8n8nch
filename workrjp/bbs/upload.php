...
$attachDir= '../upload/img/bbs';//上传文件保存路径，结尾不要带/
$dirType=1;//1:按天存入目录 2:按月存入目录 3:按扩展名存目录  建议使用按天存
$maxAttachSize=6097152;//最大上传大小，默认是2M
...
// 	session_start();
	...
	if($extension != "gif"){
		...
		$back = imagecreatefromstring(file_get_contents($tempPath));
...
if($err==''){
	...
	if(preg_match('/^('.str_replace(',','|',$upExt).')$/i',$extension))
	{
		...
		else
		{
			...
			//王一川  改
			$targetPath = APP_URL."upload/img/bbs/".$attachSubDir."/".$newFilename;
			
			//保存上传图片路径
			$photo_path="upload/img/bbs/".$attachSubDir."/".$newFilename;
			if ($_SESSION["image_path"]){
				$_SESSION["image_path"].=",".$photo_path;
			}else{
				$_SESSION["image_path"]=$photo_path;
			}
