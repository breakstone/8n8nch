<?php
	...
	if(isset($_POST['lid']) && isset($_POST['answer_ta_content']) && ((isset($_SESSION['kiwa_username'])&&isset($_SESSION['kiwa_userid'])) || (isset($_SESSION['kiwa_companyname'])&&isset($_SESSION['kiwa_companyid'])))){
		...
		$sql = "insert into dtb_bbs_answer set bbs_id = '$bbs_id',answer_user_id='$answer_user_id',answer_user_type='$answer_user_type',answer_content='$answer_content',re_answer='$re_answer',create_date='$create_date'";
		...
		$sql_s = "select user_id from dtb_bbs where bbs_id = '$bbs_id'";
			...
			header('Location:'.APP_URL.'bbs/show/'.$_POST[lid].'/?pid='.$pid.'#'.$aid);
		}
	}else{
		//没有参数跳到错误页面
