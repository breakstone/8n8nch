<?php
	...
	if(isset($_POST['lid']) && isset($_POST['answer_ta_content']) && ((isset($_SESSION['kiwa_username'])&&isset($_SESSION['kiwa_userid'])) || (isset($_SESSION['kiwa_companyname'])&&isset($_SESSION['kiwa_companyid'])))){
		...
		$sql = "insert into dtb_lives_answer set live_id = '$live_id',answer_user_id='$answer_user_id',answer_user_type='$answer_user_type',answer_content='$answer_content',re_answer='$re_answer',create_date='$create_date'";
		...
		if(!$return){
			...
			if($_POST['ta_id']!=$answer_user_id){
				...
				$message_from = "000001";
				$message_title = "系统消息,有人回复您的发表！";
				$message_content = "用户您好，有人回复您的发表！\r\n 内容: $answer_content \r\n  详细情况-> ".APP_URL."live/show/$live_id/?pid=$pid#$aid";
				$now = date("Y-m-d H:i:s");
				$message_id = date("dHis").rand(1000,9999);
			...
			header('Location:'.APP_URL.'live/show/'.$_POST[lid].'/?pid='.$pid.'#'.$aid);
		}
	}else{
		//没有参数跳到错误页面
