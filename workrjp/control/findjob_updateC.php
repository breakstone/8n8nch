<?php
	...
	//发表工作
	if(isset($_POST['people_id'])&&isset($_POST['people_title'])&&isset($_POST['people_pr'])&&isset($_SESSION['kiwa_username'])&&isset($_SESSION['kiwa_userid'])){
		//发表求职
		...
		//个人简历
		$birth = cleanInput($_POST['birthYear']."-".$_POST['birthMonth']."-".$_POST['birthDay']);
		...
		//发表求职处理
		$return1 = findjob_update($people_id,$_SESSION['kiwa_userid'],$_SESSION['kiwa_username'],$birth,$people_title,$kinds,$kindsname,$types,$typesname,$people_pr,$employid,$employname,$areaid,$pref,$prefname,$line_num,$station_cd,$station_name,$hopedateid,$hopedatename,$moneyid,$moneyname,$last_education,$last_education_name,$job_nowstatus,$job_email);
		...
		if(!$return1||!$return2){
			if(!$return1) echo "编辑求职处理 失败！" ;exit();
			if(!$return2) echo "个人简历更新 失败！" ;exit();
		}else{
			//没有参数跳到错误页面
			header('Location:'.APP_URL.'mypage/done/updateok');
		}
	}else{
		//没有参数跳到错误页面
