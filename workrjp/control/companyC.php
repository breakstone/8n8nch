<?php
	...
	//发表工作
	if(isset($_POST['password'])&&isset($_SESSION['kiwa_companyname'])&&isset($_SESSION['kiwa_companyid'])){
		...
		if(!$return){
			...
			if ($send_type==1){
				echo '<script>
				alert("修改成功");
				window.location="'.APP_URL.'people/company_pr/'.$_SESSION['kiwa_companyid'].'";
				';
				echo '</script>';
			}else{
			//没有参数跳到错误页面
			header('Location:'.APP_URL.'companypage/done/companyok/');
			}
