<?php
	...
	$lives = getLives("user_id = '$_SESSION[kiwa_companyid]'",0,12);
	...
	$sql_livep = "select * from dtb_lives where live_want = $live_want and service_id = '$service_id' $area_cd $pref_cd $line_num $station_cd $type $money $home_geju $home_mianji $home_juli $home_year $weixiu_shangmen $zhong and del_flg=0 order by create_date desc limit 0 , 10";
	$live_array = $db->QueryArray($sql_livep);

	if(count($live_array)==0){
		$sql_livep = "select * from dtb_lives where live_want = $live_want and service_id = '$service_id' $area_cd $pref_cd $type and del_flg=0 order by create_date desc limit 0 , 10";
		$live_array = $db->QueryArray($sql_livep);
	}
	...
	$sql_jobp_u = "select * from dtb_user where $kindsID $typesID and del_flg=0 limit 0 , 5";
	$job_array_u = $db->QueryArray($sql_jobp_u);
	$sql_jobp_c = "select * from dtb_companyuser where $kindsID $typesID and company_id != '$_SESSION[kiwa_companyid]' and del_flg=0 limit 0 , 5";
	$job_array_c = $db->QueryArray($sql_jobp_c);
	
	if(count($job_array_u)==0&&count($job_array_c)==0){
		$sql_jobp_u = "select * from dtb_user where $kindsID and del_flg=0 limit 0 , 5";
		$job_array_u = $db->QueryArray($sql_jobp_u);
		$sql_jobp_c = "select * from dtb_companyuser where $kindsID and company_id != '$_SESSION[kiwa_companyid]' and del_flg=0 limit 0 , 5";
		$job_array_c = $db->QueryArray($sql_jobp_c);
	}
	...
	if(isMobile()){
		$smarty->display('mobile/Companypage/companypagej.html');
	}else{
		$smarty->display('Companypage/companypagej.html');
	}
?>