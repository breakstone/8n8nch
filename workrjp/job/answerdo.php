<?php
	...
	//发表回复
	if(isset($_POST['job_id']) && isset($_POST['answer_content']) && ((isset($_SESSION['kiwa_username'])&&isset($_SESSION['kiwa_userid'])) || (isset($_SESSION['kiwa_companyname'])&&isset($_SESSION['kiwa_companyid'])))){
		//发表求职
		...
		$sql = "insert into dtb_jobs_answer set job_id = '$job_id',answer_user_id='$answer_user_id',answer_user_type='$answer_user_type',answer_content='$answer_content',create_date='$create_date'";
		...
		if(!$return){
			...
			$pid = ceil($in_id/10);
			//给发布者的信箱发送信息
			$sql_s = "select company_id from dtb_jobs where job_id = '$job_id'";
			$user = $db->QueryRow($sql_s);
			$message_to = $user["company_id"];
			if($answer_user_id != $user["company_id"]){
				$message_from = "000001";
...
// 				pointsDo($answer_user_id, $answer_user_type, 2);
			...
			header('Location:'.APP_URL.'job/show/'.$_POST[job_id].'/?pid='.$pid.'#'.$aid);
		}
	}else{
		//没有参数跳到错误页面
