...
// 	}
	...
	if(strlen($id) <= 12){
		$job = showJob($id);
		if($job){
			if(!isset($_SESSION["job_num"]) || $_SESSION["job_num"] != $job["job_id"]){
				//关注度修改
				$read_num = $job['read_num']+1;
				$sql_rn="update dtb_jobs set read_num=$read_num where job_id='$id'";
				$db->Execute($sql_rn);
				
				$_SESSION["job_num"] = $job["job_id"];
			}
			//banner随机
			...
			//分页处理
			$all_nums = getObjectNum("dtb_jobs_answer","job_id = '$id' and del_flg = 0 order by create_date");
			...
			$subPages = new SubPages($show_num,$all_nums,$pageCurrent,10,APP_URL."job/show/$id/?pid=");
			//-----------------分页处理完了----------------------
			...
			$sql = "select * from dtb_jobs_answer where job_id = '$id' and del_flg = 0 order by create_date limit $start , $end";
			...
			if(isMobile()){
				...
				$smarty->display('Job/jobshow.html');
			}
			//$smarty->display('mobile/Job/jobshow.html');
		}else{
			//没有找到工作
