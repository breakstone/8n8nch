<?php
	...
	if(isset($_POST['lid']) && isset($_POST['answer_ta_content']) && ((isset($_SESSION['kiwa_username'])&&isset($_SESSION['kiwa_userid'])) || (isset($_SESSION['kiwa_companyname'])&&isset($_SESSION['kiwa_companyid'])))){
		...
		$re_id=$_POST["aId"];
		$ta_id=$_POST["ta_id"];
		$sql = "insert into dtb_job_ls_answer set job_ls_id = '$job_id',answer_user_id='$answer_user_id',answer_user_type='$answer_user_type',answer_content='$answer_content',re_id='$re_id',re_answer_content='$re_answer',re_answer_id='$ta_id',create_date='$create_date'";
		...
		if(!$return){
			...
			header('Location:'.APP_URL.'job/lsshow/'.$_POST[lid].'/?pid='.$pid.'#'.$aid);
		}
	}else{
		//没有参数跳到错误页面
