	...
	if($_POST["id"]){
		...
		if(isset($_POST["id"])&&$_POST["id"]=="141113009425"){//喜和工业
			if(isset($_POST["kid"])&&isset($_POST["password"])){
				...
				if($staff["staff_id"]!=""){
					$sqlgz = "select * from kiwa_mingxi where staff_id='$manager_id' and delflag = 0";
		...
		if(isset($_POST["id"])&&$_POST["id"]=="071445274801"){//WAYS
			if(isset($_POST["kid"])&&isset($_POST["password"])){
				...
				if($staff["staff_id"]!=""){
					$sqlgz = "select * from ways_mingxi where staff_id='$manager_id' and delflag = 0";
					$gongzi = $db->QueryRow($sqlgz);
					$shijian=$gongzi["moneytime"];
					$shi= explode(".",$shijian);
					$fen=($shijian-$shi[0])*60;
		...
		if(isset($_POST["id"])&&$_POST["id"]=="161559139290"){//ESS
			if(isset($_POST["kid"])&&isset($_POST["password"])){
				...
				if($staff["staff_id"]!=""){
					$sqlgz = "select * from ess_mingxi where staff_id='$manager_id' and delflag = 0";
					$gongzi = $db->QueryRow($sqlgz);
					$shijian=$gongzi["moneytime"];
					$shi= explode(".",$shijian);
					$fen=($shijian-$shi[0])*60;
