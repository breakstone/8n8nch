	...
	if(strlen($id[1]) <= 12){
		...
		if($id[0] == "company"){
			...
			if($company){
				...
				$subPages = new SubPages($show_num,$all_nums,$pageCurrent,10,APP_URL."people/msg/company_$id[1]/?page=");
				//-----------------分页处理完了----------------------
				...
				$msgs = getMsg("people_id='$id[1]'",$start, $end);
				...
				$smarty->display('People/company_msg.html');
			}else{
				//没有找到
		...
		if($id[0] == "user"){
			$user = getUser($id[1]);
			if($user){
				$subPages = new SubPages($show_num,$all_nums,$pageCurrent,10,APP_URL."people/msg/user_$id[1]/?page=");
				//-----------------分页处理完了----------------------
				//利用limit查询数据库---select * from table limit $start,$end
				$start = $subPages->getStart($show_num,$pageCurrent);//配合分页得到起始数
				$end = $subPages->getEnd($all_nums,$show_num,$pageCurrent);//配合分页得到结束数
				...
				$smarty->assign('BASE_URL', APP_URL);
				$smarty->assign('THEME', THEME);
				$smarty->display('People/people_msg.html');
			}else{
				//没有找到
				$smarty->assign('h1', "信息提示");
