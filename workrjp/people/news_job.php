	...
	if(strlen($id[1]) <= 12){
		...
		if($id[0] == "company"){
			...
			if($company){
				...
				$subPages = new SubPages($show_num,$job_nums,$pageCurrent,10,APP_URL."people/job/company_$id[1]/?page=");
				//-----------------分页处理完了----------------------
				//利用limit查询数据库---select * from table limit $start,$end
				$start = $subPages->getStart($show_num,$pageCurrent);//配合分页得到起始数
				$end = $subPages->getEnd($job_nums,$show_num,$pageCurrent);//配合分页得到结束数
				$smarty->assign('subPages',$subPages->showPageStr());
				
				$jobs = getJobs("company_id='$id[1]'",$start, $end);
				...
				$smarty->display('People/company_job.html');
			}else{
				//没有找到
		...
		if($id[0] == "user"){
			$user = getUser($id[1]);
			if($user){
				$subPages = new SubPages($show_num,$job_nums,$pageCurrent,10,APP_URL."people/job/user_$id[1]/?page=");
				//-----------------分页处理完了----------------------
				//利用limit查询数据库---select * from table limit $start,$end
				$start = $subPages->getStart($show_num,$pageCurrent);//配合分页得到起始数
				$end = $subPages->getEnd($job_nums,$show_num,$pageCurrent);//配合分页得到结束数
				...
				$smarty->assign('jobs', $jobs);
				if (empty($jobs)){
					$jobs_tj = getJobs("",0, 10);
					$smarty->assign('jobs_tj', $jobs_tj);
				}
				//人信息
