...
if(strlen($id[1]) <= 12 && ((@$_SESSION["kiwa_userid"] == $id[1]) || (@$_SESSION["kiwa_companyid"] == $id[1]))){
	...
	if($id[0] == "company"){
		...
		if($company){
			...
			if(isMobile()){
				$smarty->display('mobile/People/company_photoadd.html');
			}else{
				$smarty->display('People/company_photoadd.html');
			}
	...
	if($id[0] == "user"){
		...
		if($user){
			...
			$smarty->assign('THEME', THEME);
			if(isMobile()){
				$smarty->display('mobile/People/people_photoadd.html');
			}else{
				$smarty->display('People/people_photoadd.html');
			}
